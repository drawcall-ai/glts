...
export class Operations {
  ...
  runLoad<T>(url: string, operation: () => Promise<T>): Promise<T> {
    return this.#withLock(url, () => this.#locks.acquireRead(url), operation);
  }

  runNestedLoad<T>(url: string, operation: () => Promise<T>): Promise<T> {
    return this.#withLock(
      url,
      () => this.#locks.acquireReadBeforeQueuedWrite(url),
      operation
    );
  }

  runReload<T>(url: string, operation: () => Promise<T>): Promise<T> {
    return this.#withLock(url, () => this.#locks.acquireWrite(url), operation);
  }
