...
import { LoadingScope } from "../../src/loader/loading.js";
...
describe("LoadingScope", () => {
  it("waits only for resources started through its manager", async () => {
    ...
    const firstCompletion = first.waitAndClose().then(() => {
      firstSettled = true;
    });
    await expect(second.waitAndClose()).resolves.toBeUndefined();
  ...
  it("isolates a resource failure from other scopes", async () => {
    ...
    await expect(successful.waitAndClose()).resolves.toBeUndefined();
    await expect(failed.waitAndClose()).rejects.toMatchObject({
      phase: "resource",
      url: "failed.glts"
    });
  });
  ...
  it("balances host notifications when callbacks throw", async () => {
    ...
    await expect(scope.waitAndClose()).rejects.toMatchObject({
      phase: "resource",
      url: "tree.glts"
    });
  ...
  it("rejects pending completion when cancelled", async () => {
    ...
    const completion = scope.waitAndClose();
